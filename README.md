...
$$\text{Base XP} = (\text{MCQs Completed} \times 10) + (\text{Minutes Studied} \times 1)$$

$$\text{Multiplier} = \begin{cases} 1.2 & \text{if Accuracy} \ge 80\% \\ 1.0 & \text{otherwise} \end{cases}$$

$$\text{XP Earned} = \text{round}(\text{Base XP} \times \text{Multiplier})$$

* **Level Progression**: A user advances to level $L+1$ when total XP reaches $L \times 1000$.
* **Rank Tiers**:
...
* **Streak Calculation**: Evaluates daily activity timestamps. If the difference between current session and last active date is $\le 1 + \text{STREAK\_GRACE\_DAYS}$ (1 day grace), the streak increments; otherwise, it resets cleanly to 1.
...
$$\text{Accuracy Score} = \left(\frac{\text{Avg Accuracy}}{100}\right)^3 \times 70 \quad (\text{Cubic scaling enforces precision})$$

$$\text{Volume Score} = \min\left(\frac{\text{Total MCQs}}{5000}, 1\right) \times 20$$

$$\text{Consistency Score} = \min\left(\frac{\text{Current Streak}}{15}, 1\right) \times 10$$

$$\text{Weak Subject Penalty} = \max(0.75, 1 - (\text{Weak Subjects Count} \times 0.05))$$

$$\text{Readiness (\%)} = \text{round}((\text{Accuracy} + \text{Volume} + \text{Consistency}) \times \text{Weak Penalty})$$

$$\text{Predicted GMR} = \max\left(1, \min\left(25000, \text{round}\left(25000 \times \left(1 - \frac{\text{Readiness}}{100}\right)^{2.2}\right)\right)\right)$$
...
* **Multi-Stage Celebrations**: Embedded canvas confetti triggers standard bursts for $\ge 80\%$ accuracy and a high-intensity firecracker ("Patakha") loop for scores $\ge 90\%$.
...
* **365-Day Activity Heatmap**: Visualizes annual preparation density on a 5-tier intensity scale ($0 = \text{0 MCQs}, 1 = <20, 2 = <50, 3 = <100, 4 = 100+$).
...
│       │   │   ├── mistakes.controller.js
│       │   │   ├── mistakes.model.js
│       │   │   ├── mistakes.routes.js
│       │   │   ├── mistakes.service.js
│       │   │   └── mistakes.validation.js
...
- `userId` (ObjectId $\to$ User, indexed): Owner of the log.
...
- `userId` (ObjectId $\to$ User, indexed): Owner.
